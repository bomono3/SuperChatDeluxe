...
public interface MessageRepository extends JpaRepository<Message, Integer>{
	...
	@Query(value = "select * from message where (username is not null and (is_private = true and (username != ?1 and sent_to = ?1))) or (username is not null and is_private = false) order by time_sent asc", nativeQuery = true)
	public List<Message> getMessagesByLatest(String username);
	
	@Query(value = "select * from message where (username is not null and (is_private = true and (username != ?1 and sent_to = ?1))) or (username is not null and is_private = false) order by time_sent desc limit ?2", nativeQuery = true)
