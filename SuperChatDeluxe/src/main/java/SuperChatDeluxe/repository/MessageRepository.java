...
public interface MessageRepository extends JpaRepository<Message, Integer>{
	...
	@Query(value = "select message from message where username is not null and (is_private = true and (username = ?1 or sent_to = ?1)) order by time_sent desc", nativeQuery = true)
	...
	@Query( value = "select * from message where username is not null and (is_private = true and (username = ?1 or sent_to = ?1)) and between ?2 and ?3 order by time_sent desc" , nativeQuery = true)
